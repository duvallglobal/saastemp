...
// Create route imports for client pages
const ClientDashboardPath = "/_app/_auth/client";
const ContentUploadPath = "/_app/_auth/client/content-upload";
const AppointmentApprovalPath = "/_app/_auth/client/appointment-approval";
const SubscriptionPath = "/_app/_auth/client/subscription";
const ProfilePath = "/_app/_auth/client/profile";

export function ClientNavigation({ user }: { user: UserType }) {
  const signOut = useSignOut();
  const matchRoute = useMatchRoute();
  const navigate = useNavigate();
  
  const isDashboardPath = matchRoute({ to: ClientDashboardPath });
  const isContentUploadPath = matchRoute({ to: ContentUploadPath });
  const isAppointmentApprovalPath = matchRoute({ to: AppointmentApprovalPath });
  const isSubscriptionPath = matchRoute({ to: SubscriptionPath });
  const isProfilePath = matchRoute({ to: ProfilePath });
    ...
    <nav className="sticky top-0 z-50 flex w-full flex-col border-b border-border bg-card px-6">
      ...
      <div className="mx-auto flex w-full max-w-screen-xl items-center gap-3">
        ...
        <div
          className={cn(
            `flex h-12 items-center border-b-2`,
            isContentUploadPath ? "border-primary" : "border-transparent",
          )}
        >
          <Link
            to={ContentUploadPath}
            className={cn(
        ...
        <div
          className={cn(
            `flex h-12 items-center border-b-2`,
            isAppointmentApprovalPath ? "border-primary" : "border-transparent",
          )}
        >
          <Link
            to={AppointmentApprovalPath}
            className={cn(
              `${buttonVariants({ variant: "ghost", size: "sm" })} text-primary/80 flex items-center gap-1`,
            )}
          >
        ...
        <div
          className={cn(
            `flex h-12 items-center border-b-2`,
            isSubscriptionPath ? "border-primary" : "border-transparent",
          )}
        >
          <Link
            to={SubscriptionPath}
        ...
        <div
          className={cn(
            `flex h-12 items-center border-b-2`,
            isProfilePath ? "border-primary" : "border-transparent",
          )}
        >
          <Link
            to={ProfilePath}
